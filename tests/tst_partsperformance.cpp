...
private slots:
    ...
    void zimaArchivesAreIndependent()
    {
        ...
        QCOMPARE(FileMetadata("item.prtz").type, FileType::ZIMA_PART);
        QCOMPARE(FileMetadata("item.prtz.10").type, FileType::ZIMA_PART);
        QCOMPARE(FileMetadata("item.ASMZ.2").type, FileType::ZIMA_ASSEMBLY);
        QCOMPARE(FileMetadata("item.drwz").type, FileType::ZIMA_DRAWING);
